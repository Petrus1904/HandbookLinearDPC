%% Intro
% Predictive control of a plane model using the
...
Ry = [zeros(0*ny,1); kron(ones(N,1), [10;0])];
Ru = zeros(N*nu, 1);
uu = Gi*Gamma'*Omega*Ry;
yy = Gamma*uu;
HankeRausRegularization([chol(Omega)*Yf_g; chol(Psi)*Uf_g], [chol(Omega)*(Ry-yy); chol(Psi)*(Ru-uu)], [1e-2, 1e8, 1000], l_g);
...
objective = objective +l_g*(g'*g); % cost for DeePC regularization terms

% Build constraints
constraints = [zeros(nu*Tini,1)==Up_g*g, zeros(ny*Tini,1)==Yp_g*g, y-y_spc==Yf_g*g, u-u_spc==Uf_g*g];
