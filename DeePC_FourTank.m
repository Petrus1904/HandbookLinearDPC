%% Intro
% Data-EnablEd Predictive Control of the Four tank system. Considers l2
% regularization on g.
...
%I set this to 2500 to not mess with the RNG wrt the other simulations
[sys, constr, Hankel] = GetDataFourTankModel(2500, N, Tini, wvar, Ts);
...
% HankeRausRegularization([chol(Omega)*Yf; chol(Psi)*Uf]*Wp0, [chol(Omega)*Ry; chol(Psi)*Ru], [1e-4, 1e8, 1000]);
...
disp(['DeePC ISE =', num2str(ISE), ' IAE = ', num2str(IAE), ' INEN = ', num2str(INEN), ' cpu = ', num2str(mean(cpu))]);
