%% Intro
% MPC simulation of a Four tank model. Considers classic MPC control law
...
% dual problem. Data acquisition and simulation are subject to sensor noise
...
%this gave the best fit in the sysid toolbox
% SYSpoly = armax(Iddat, [3*ones(2,2), 3*ones(2,2), 3*ones(2,1), 1*ones(2,2)]);
% SYSid = ss(SYSpoly);
SYSid = n4sid(Iddat, 4); %yes I do assume perfect prediction, although Sys id toolbox suggests 3
...
Aid = SYSid.A;
nid = size(Aid,1);
Kid = dlqr(Aid', Cid', eye(nid), 10*eye(ny))';
...
disp(['MPC ISE =', num2str(ISE), ' IAE = ', num2str(IAE), 'INEN = ', num2str(INEN)]);
