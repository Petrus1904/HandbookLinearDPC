%% Intro
% Data-EnablEd Predictive Control of the plane model. Consider
% PI-regularization on g.
...
[sys, constr, Hankel] = GetDataPlaneModel(T*(2)*(Tini+N), N, Tini, wvar, Ts);
...
Up = Hankel.Up; %(:, 1:nu*(Tini+N):end)
Yp = Hankel.Yp; %(:, 1:ny*(Tini+N):end)
Uf = Hankel.Uf; %(:, 1:nu*(Tini+N):end)
Yf = Hankel.Yf; %(:, 1:ny*(Tini+N):end)
...
% Ry = wvar*randn(ny*N,1); %kron(ones(N,1), [10;0]);
% Ru = 0*kron(ones(N,1), u_ss);
% HankeRausRegularization([chol(Omega)*Yf; chol(Psi)*Uf]*L, [chol(Omega)*Ry; chol(Psi)*Ru], [1e-4, 1e8, 1000]);
...
objective = objective +l_g*(g'*L'*L*g); % + l_y*(sigma_y'*sigma_y); % cost for DeePC regularization terms

% Build constraints
constraints = [u_ini==Up*g, y_ini==Yp*g, y==Yf*g, u==Uf*g];
% constraints = [u_ini==Up*g, y_ini+sigma_y==Yp*g];
...
Outputs = {u, y, g};
options = sdpsettings('solver', 'quadprog', 'verbose', 0, 'debug', 0, 'osqp.eps_abs', 1e-8, 'osqp.eps_rel', 1e-8);
...
stairs(t, y(1,:), 'r', 'DisplayName', ['DeePC PI y1, T=', num2str(T)]);
stairs(t, y(2,:), 'b', 'DisplayName', 'DeePC PI y2');
ylabel('Output [-]');
legend;
grid on; grid minor;
axis([0 t(end) -1 15]);
...
disp(['DeePC PI control cost =', num2str(CtrlCost)]);
